...
// ------
//cambiar
import { delet, getVer, getListaPaginate, postGuardar } from '@/app/panel-control/services/SliderServices'
import type { Slider } from "../../interface/Slider"
...
const CardTable = () => {
  ...
  */
  ...
  const [formData, setFormData] = useState({ nombre: "", id: 0, imagen: null as File | null | string, url:"" }); // variable donde se guarda el formulario 
  const [dataJson, setDataJson] = useState<Slider[]>([]); // donde se guarda el json que se trae del backend
  ...
  */
  ...
  const handleEdit = async (id: number) => {
    const respons: Slider = await getVer(id);
  ...
  return (
    <ComponentCard title="Lista de Sliders">
...
const ListSliders = () => {
  return (
    <Container fluid>
      <PageBreadcrumb title="Gestion de Sliders" subtitle="" />
